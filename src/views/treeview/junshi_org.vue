            ...
            <el-container>
                ...
                <el-main v-if="weiboData.length > 0" style="max-height: 500px">
                    <el-descriptions v-for="(weibo, index) in weiboData" :key="index" column="1">
                        ...
                        <el-descriptions-item label="国际名称">{{ id2NodeInfo.map[weibo.id].i18nName }}</el-descriptions-item>
                        <el-descriptions-item label="成立时间">{{ id2NodeInfo.map[weibo.id].time }}</el-descriptions-item>
                        <el-descriptions-item label="总&ensp;&ensp;&ensp;&ensp;部">{{ id2NodeInfo.map[weibo.id].main }}</el-descriptions-item>
                        <el-descriptions-item label="成员信息">{{ id2NodeInfo.map[weibo.id].member }}</el-descriptions-item>
                        <el-descriptions-item label="别&ensp;&ensp;&ensp;&ensp;名">{{ id2NodeInfo.map[weibo.id].alias }}</el-descriptions-item>
                        <el-descriptions-item label="&ensp;领导人">{{ id2NodeInfo.map[weibo.id].header }}</el-descriptions-item>
                    </el-descriptions>
                </el-main>
            </el-container>
        </page-main>
...
export default defineComponent({
    ...
    setup() {
        ...
        onMounted(() => {
            getTreeList(graphId, 1, 2, 0, 0, '').then(res => {

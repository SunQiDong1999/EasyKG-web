<template>
    <div>
        <page-main>
            ...
            <el-container>
                <el-main v-if="aside.cur === '实体信息'">
                    <el-descriptions v-if="entityInfo.labels[0] === 'WeiboTag'" border :column="1">
                        <el-descriptions-item label="话题">{{ entityInfo.info["name"] }}</el-descriptions-item>
                    </el-descriptions>
                    <el-descriptions v-if="entityInfo.labels[0] === 'Weibo'" border :column="1">
...
import {
    ...
    removeEntity,
    createEntity,
    getEntityDegreeNeighbors,
    getEntityNeighborsWithPagination
} from '@/api/graph'
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            const menu = new G6.Menu({
                ...
                handleMenuClick(target, item) {
                    ...
                    if (cal === 'DFS') {
                        ...
                        // 下翻
                        let nodeId = item._cfg.model.id
                        let nextPage = pages.curPages[nodeId] + 1
                        let newNodes
                        let tobeDeletedNodeIds
                        newNodes = []
                        tobeDeletedNodeIds = []
                        const { depthFirstSearch } = Algorithm
                        depthFirstSearch(g6Data, item._cfg.model.id, {
                            enter: ({ current, previous }) => {
                                console.log('进入', current, previous)// 开始遍历点的回调
                                tobeDeletedNodeIds.push(current)
                            },
                            leave: ({ current, previous }) => {
                                console.log('离开', current, previous)// 遍历完节点的回调
                            }
                        })
                        tobeDeletedNodeIds = tobeDeletedNodeIds.slice(1)
                        g6Data.nodes.forEach(node => {
                            if (tobeDeletedNodeIds.indexOf(node.id) === -1) {
                                newNodes.push(node)
                            }
                        })
                        getEntityNeighborsWithPagination(graphId, nodeId, 3, nextPage).then(res => {
                            g6Data.nodes = uniqueFunc(newNodes.concat(res.data), 'id')
                            getRelationsOfEntities(graphId, g6Data.nodes).then(res => {
                                g6Data.edges = res.data
                                setG6Data(g6Data)
                                g6Graph.value.render() // 读取 Step 2 中的数据源到图上
                        ...
                        pages.curPages[nodeId] = pages.curPages[nodeId] + 1
                    } else if (cal === 'prev') {
                        // 上翻
                        let nodeId = item._cfg.model.id
                        let nextPage = pages.curPages[nodeId] - 1
            ...
            getGraphById(graphId).then(res => {
                ...
                // 初始化
                getEntities(graphId, pagination.size, pagination.page).then(res => {
                    g6Data.nodes = uniqueFunc(res.data.data, 'id')
                    g6Data.nodes.forEach(node => {
                        pages.curPages[parseInt(node.id)] = 1
                    })
                    console.log(g6Data.nodes)
                    getRelationsOfEntities(graphId, g6Data.nodes).then(res => {
                        g6Data.edges = res.data

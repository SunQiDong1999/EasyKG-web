...
const useRouteMainStore = defineStore(
    // 唯一ID
    'routeMain',
    {
        state: () => ({
        ...
        actions: {
            // 根据权限动态生成路由（前端生成）
            generateMainRoutesAtFront(mainRoutes) {
                ...
                return new Promise(async resolve => {
                    ...
                    if (settingsStore.app.enablePermission) {
                        ...
                        accessedRoutes = filterAsyncRoutes(mainRoutes, permissions)
                    } else {
                        accessedRoutes = deepClone(mainRoutes)
                    }
                    // 设置 routes 数据
...
export default useRouteMainStore
